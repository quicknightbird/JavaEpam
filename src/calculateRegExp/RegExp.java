...
public class RegExp {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(19 + 16 * 11 + (12 - 16) * 10 + 444 - 32 + 2 * (-12) * (3 + 2));
        String InputExp1 = "1*2*3*4*5";
        System.out.println(2*3*4*5);
        System.out.println(Calculate(CheckBrackets(InputExp)));
        System.out.println(Calculate(CheckBrackets(InputExp1)));
        Scanner in = new Scanner(System.in);
        String expression = in.nextLine();
        System.out.println(Calculate(CheckBrackets(expression)));
    }

    public static String CheckBrackets(String InputExp) {           // соответсвенно просчитывает через клкл скобки и возвращает выражение с решением скобками
    ...
    public static int Calculate(String InputExp) {   // cчитает и скобки и само выражение, которое подается на вход строкой
        Matcher matcher = NUMBER.matcher(InputExp);
        Matcher matcherChar = SUM.matcher(InputExp);
        Stack<Integer> stackNumbers = new Stack<Integer>();   // два стека один для чиселок другой для операций
